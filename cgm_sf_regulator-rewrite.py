...
def halo_mass_growth(t, mass):
    ...
    mhalo = mass * u.solMass
    #    z = np.sqrt((28/t) - 1) - 1 # cosmological redshift
    if t > 13.4:  # 13.4 is the age of the universe in Gyr
        t = 13.4
    z = cosmology.z_at_value(LCDM.age, t * u.Gyr)
    mhalo_dot = halo_infall_dekel(z, mhalo)
...
def plot_halo_diagnostics(results, derived_quant, eta_m=0.5, eta_e=0.1, eta_z=0.3):
    ...
    ax[3].set(yscale="log", ylabel=r"$E ~ [{\rm erg}]$", ylim=(1e50, 4e55))
    ax[3].legend(ncols=2, frameon=False)
    ax[4].plot(t, f_star)
    ax[4].set_yscale("log")
...
def custom_energy_loading(mhalo, A=0.15, alpha=-0.5):
    """energy loading factor as a function of halo mass"""
    return A * (mhalo / (1e12 * u.solMass)) ** alpha
...
# %%
zobs = [11, 10, 9]
mhalos = np.geomspace(1e8, 1e11, 10) * u.Msun
redshift_variation, _ = run_redshift_grid(
    zobs, mhalos, write_to_file="./runs/smhm_grid_varying_etaEandetaM_extended.dat"
)
...
for i, smhm_model in enumerate(redshift_variation):
    ax.plot(mhalos, smhm_model, markersize=3, marker="o", label=r"$z$ = {:.1f}".format(zobs[i]))

# model text
model_text = (
    r"$\eta_m = 10 \left( M_{\rm halo} / 10^{10} M_\odot \right)^{-0.7}$"
    + "\n"
    + r"$\eta_e = 0.15 \left( M_{\rm halo} / 10^{12} M_\odot \right)^{-0.5}$"
)
ax.text(
    0.95,
    0.1,
...
ax.legend(bbox_to_anchor=(0.45, 1.01), loc="lower center", ncol=3, frameon=False)
plt.show()
# %% vary the eta_m
