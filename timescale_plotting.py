...
if PLOT_TWO_PANEL:
    fig, ax = plt.subplots(
        4, 2, figsize=(8.5, 8.5), dpi=300, sharex="col", sharey="row"
    )
...
for res in halo_results:
    ...
    mask = t_vir > threshold
    ax_row = (
        [ax[0, 0], ax[1, 0], ax[2, 0], ax[3, 0]]
        if PLOT_TWO_PANEL
        else [ax[0], ax[1], ax[2], ax[3]]
    )

    if np.sum(mask) > 0:
        begin = np.argmax(np.argwhere(t_vir < threshold).flatten())
        ax_row[0].plot(
            times[: begin + 2], y_cool[: begin + 2], ls="-", lw=2, color=color
        )
        ax_row[1].plot(
            times[: begin + 2], y_eject[: begin + 2], ls="-", lw=2, color=color
        )
        ax_row[2].plot(times[: begin + 2], y_in[: begin + 2], ls="-", lw=2, color=color)
        ax_row[3].plot(
            times[: begin + 2], y_wind[: begin + 2], ls="-", lw=2, color=color
        )
...
if PLOT_TWO_PANEL:
    for res in halo_results_baseline:
        ...
        if np.sum(mask) > 0:
            ...
            ax[2, 1].plot(
                times[: begin + 2], y_in[: begin + 2], ls="-", lw=2, color=color
            )
            ax[3, 1].plot(
                times[: begin + 2], y_wind[: begin + 2], ls="-", lw=2, color=color
            )
