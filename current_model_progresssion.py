...
# write to file
file = (
    "./runs/smhm_baseline_redshift_16halos_infallupdated_loadingsupdated_SFupdated.h5"
)
...
# %%
run_progressions = [
    "./runs/smhm_baseline_redshift_16halos_infallupdated.h5",
    "./runs/smhm_baseline_redshift_16halos_infallupdated_loadingsupdated.h5",
    "./runs/smhm_baseline_redshift_16halos_infallupdated_loadingsupdated_SFupdated.h5",
]
run_name=["halo accretion rates", "+ mass and energy loading", "+ star formation rates"]

fig, ax = plt.subplots(2, len(run_progressions), figsize=(11, 6.25), dpi=300, sharey="row")
plt.subplots_adjust(hspace=0.4, wspace=0.05)
for col, file in enumerate(run_progressions):
    f = h5py.File(file, "r")
    smhm_normalized = f["SMHM"]  # smhm is already normalized by baryon fractions
    mhalo_obs = f["Mhalo_obs"]
    print(f"File {col}: {file}")
    print(f.keys())
    print(f["redshifts"][:])
    ...
    if col==1:
        ax[1, col].set( xlabel=r"$M_{\rm halo}$ [M$_\odot$]")
        ax[0, col].set( xlabel=r"$T_{\rm vir}$ [K]")
        

    # Add a twin x-axis to ax[0, col] showing circular velocity corresponding to T_vir

    # Get current x-ticks for T_vir axis
    tvir_ticks = ax[0, col].get_xticks()
    # Remove ticks outside the plotting range
    tvir_ticks = tvir_ticks[
        (tvir_ticks >= ax[0, col].get_xlim()[0]) & (tvir_ticks <= ax[0, col].get_xlim()[1])
    ]
    ...
    # Create twin axis
    ax0_vcirc = ax[0, col].twiny()
    ax0_vcirc.set_xscale("log")
    ax0_vcirc.set_xlim(ax[0, col].get_xlim())
    ...
    ax[1, col].plot(mhalo_obs[-1][mask], smhm_normalized[-1][mask], color="grey", lw=2, alpha=1)
    ax[1, col].plot(
        mhalo_obs[-1][~mask],
    ...
    # add text indicating the model progression on the top left
    ax[0, col].text(
        0.05, 0.95, run_name[col], transform=ax[0, col].transAxes, ha="left", fontsize=10, va="top"
    )


ax[0,0].set( ylabel=r"$M_{\star}$/ $M_{\rm halo} (\Omega_{\rm b}/\Omega_{\rm m})$")
ax[1,0].set( ylabel=r"$M_{\star}$/ $M_{\rm halo} (\Omega_{\rm b}/\Omega_{\rm m})$")
ax[1, 1].legend(
    frameon=False,
    fontsize=9,
    ncol=6,
    # title="Shuntov et. al. 24",
    title_fontsize=10,
    bbox_to_anchor=(0.43, 1.23),
    loc="upper center",
)
ax[0, 1].legend(
    frameon=False,
...
#     # Get current x-ticks for T_vir axis
#     tvir_ticks = ax[0].get_xticks()
#     # Remove ticks outside the plotting range
#     tvir_ticks = tvir_ticks[
#         (tvir_ticks >= ax[0].get_xlim()[0]) & (tvir_ticks <= ax[0].get_xlim()[1])
#     ]
...
#     # ax[0].text(
#     #     0.1, 0.9, models_label, transform=ax[0].transAxes, ha="left", fontsize=10, va="top"
#     # )
