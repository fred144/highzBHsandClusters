...
f_prevent = 0.5
alpha = 1.4  # density profile slope
metallicity = 0.01  # in solar units
dot_m_star = 1e7 * u.Msun / u.Gyr
m_halo = 1e11 * u.Msun

m_cgm_hot_range = np.linspace(8, np.log10(0.8*m_halo.value), n) 
T_CGM_range = np.linspace(4.6, 7, n)

M_grid, T_grid = np.meshgrid(m_cgm_hot_range, T_CGM_range)

# Calculate dotE_CGM for each combination
dotE_grid = np.zeros((n, n))
for i in range(n):
    for j in range(n):
        dotE_SNe = dotE_SNe_wind(dot_m_star, m_halo)
        dotE_acc = dotE_CGM_acc(m_halo, halo_Tvir_K)
        dotE_ej = dotE_CGM_ej(
            10**M_grid[i, j]* u.Msun, halo_rvir_kpc, m_halo, 10**T_grid[i, j]* u.K, halo_Tvir_K
        )
...
norm = SymLogNorm(linthresh=vmax * 1e-6, vmin=-vmax, vmax=vmax)
...
# color bars
desired_ticks = 9
locator = SymmetricalLogLocator(linthresh=vmax * 1e-6, base=10)
