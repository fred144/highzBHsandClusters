...
def dotE_CGM_ej(m_cgm_hot, halo_rvir_kpc, mhalo, T_CGM, T_vir):
    ...
    E_CGM = consts.k_B / (mu * consts.m_p) * m_cgm_hot * T_CGM 
    E_vir = consts.k_B / (mu * consts.m_p) * m_cgm_hot * T_vir
    # dotE = consts.k_B / (mu * consts.m_p) * (m_cgm_hot / t_eject) * (T_CGM - T_vir)
    
    dotE = (E_CGM - E_vir) / t_eject
    
    # ejective energy can only be positive
    if np.any(dotE < 0):
        dotE = np.where(dotE < 0, 0 * u.erg / u.Gyr, dotE)
...
Tvir_actual = derived_["halo_vir_temp"]  ##
...
ax[4].plot(time_gyr, Tcgm_actual, color="tab:orange", lw=lw, label=r"$T_{\mathrm{CGM}}$")
ax[4].plot(time_gyr, Tvir_actual, color="tab:blue", lw=lw, label=r"$T_{\mathrm{vir}}$")
ax[4].set(
    xscale="log",
    yscale="log",
    xlabel=r"$t$ [Gyr]",
...
# color bar for inset
vmax = max(abs(dotE_grid_inset_normalized.max()), abs(dotE_grid_inset_normalized.min()))

linthresh = vmax * lin_threshhold_factor
norm_new = SymLogNorm(linthresh=linthresh, vmin=-vmax, vmax=vmax)
