...
egy_cgm_in_prefac = np.linspace(0.1, 1, 20)
xlim_for_zoom = (0.166, 2)

for fp in egy_cgm_in_prefac[:1:-1]:
    ...
    model_2Phase_long = CGMRegulator(
        mhalo_z0, (0.15, 13), KS_kappa_s=0.1, add_f_prevent_floor=1e-6, verbose=False, dbug_norm_for_accretion_energy_rate=fp
    )
    ...
    ax2.plot(results_2phase["t"], derived_2phase["cgm_temp"], color="crimson", lw=3, label=r"$T_{\rm CGM}$")
    
    # we can also plot the virial temperature for reference: halo_vir_temp
    ax2.plot(
        results_2phase["t"], derived_2phase["halo_vir_temp"], color="forestgreen", lw=3, ls="--", label=r"$T_{\rm vir}$"
    )
    ...
    axin.text(
        0.05,
        0.85,
        r"fraction of E_CGM_in = {:.6f}".format(round(fp, 6)),
    ...
    plt.savefig(
        "./figures/removing_egy_from_acc_1e12/2phase_detailed_cgm_1e12_prefac_{:.6f}.png".format(
            fp
        ),
        dpi=200,
        bbox_inches="tight",
